...
terminal_screen_update_font (TerminalScreen *screen)
{
  ...
  if (TERMINAL_IS_WINDOW (toplevel) && terminal_window_get_font (TERMINAL_WINDOW (toplevel)) != NULL)
    {
      font_name = g_strdup (terminal_window_get_font (TERMINAL_WINDOW (toplevel)));
    }
  else if (font_use_system)
    {
      /* read Xfce settings */
      if (xfconf_init (NULL))
        {
          ...
          if (xfconf_channel_has_property (channel, "/Gtk/MonospaceFontName"))
            font_name = xfconf_channel_get_string (channel, "/Gtk/MonospaceFontName", NULL);
          xfconf_shutdown ();
        }
